...
func BenchmarkRangeScans(b *testing.B, db tmdb.DBReadWriter, dbSize int64) {
	b.StopTimer()
...
func BenchmarkRandomReadsWrites(b *testing.B, db tmdb.DBReadWriter) {
	b.StopTimer()
...
func TestGetSetHasDelete(t *testing.T, conn tmdb.DB) {
	{
		txn := conn.NewReaderAt(conn.CurrentVersion())
...
func TestVersioning(t *testing.T, conn tmdb.DB) {
	...
	db.Commit()
	saved := conn.SaveVersion()

	db.Set([]byte("0"), []byte("c"))
	db.Delete([]byte("1"))
	db.Set([]byte("2"), []byte("c"))

	view := conn.NewReaderAt(saved)
	...
	// Try invalid version
	view = conn.NewReaderAt(conn.CurrentVersion() + 1)
	require.Nil(t, view)
}
